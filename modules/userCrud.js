...
router.post('/users', async (req, res) => {
    ...
    if (auth !== undefined) {
        ...
        if (findUserId.role === "ADMIN") {
            ...
            res.status(403).send("Forbidden: you'r role isn't admin and you can't create user")
        }
    } else {
        res.status(401).send("Unauthorized: Please login and try again !")
    }
...
router.put('/users', async (req, res) => {
    ...
    if (auth !== undefined) {
        ...
        if (findUserId.role === "ADMIN") {
            ...
            res.status(403).send("Forbidden: you'r role isn't admin and you can't edit user")
        }
    } else {
        res.status(401).send("Unauthorized: Please login and try again !")
    }
...
router.delete('/users/:id', async (req, res) => {
    ...
    if (auth !== undefined) {
        ...
        if (findUserId.role === "ADMIN") {
            ...
            res.status(403).send("Forbidden: you'r role isn't admin and you can't delete user")
        }
    } else {
        res.status(401).send("Unauthorized: Please login and try again !")
    }
